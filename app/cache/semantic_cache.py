"""Semantic Cache Manager orchestrating ChromaDB and Sentence Transformers."""
...
class SemanticCacheManager:
    ...
    Enforces cosine similarity threshold (>= 0.90) and API-key tenant isolation.
    """
    ...
    def __init__(
        ...
        self.vector_store = vector_store or ChromaVectorStore(isolate_by_api_key=settings.CACHE_ISOLATE_BY_API_KEY)
    ...
    async def lookup(
        ...
        # 1. Exact match fast path (sub-millisecond)
        exact_item = self.vector_store.get_by_exact_match(
            prompt=user_prompt,
            model=model,
